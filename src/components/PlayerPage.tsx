...
import { ArrowLeft, Hash, Zap, Gamepad2, Trophy } from "lucide-react";
import { useTranslation } from "react-i18next";
import { RatingChart } from "@/components/RatingChart";
import { RatingPrediction } from "@/components/RatingPrediction";
import { PinButton } from "@/components/PinButton";
import { fetchLeaderboard, fetchRatingHistory } from "@/lib/api";
...
export function PlayerPage({ params, seasonId: activeSeason }: { params: { username: string }; seasonId: number | null }) {
  ...
  const { data: ratingData } = useFetch(
    () =>
      activeSeason != null
        ? fetchRatingHistory(username, activeSeason)
        : Promise.resolve(null),
    [activeSeason, username]
  );
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm h-8">
          ...
          {playerEntry ? (
            <>
              ...
              {ratingData && ratingData.estimatedGames > 0 && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-card/50 border border-border/40 font-mono">
                  <Gamepad2 className="w-3.5 h-3.5 text-amber-500" />
                  <span className="text-foreground font-bold tabular-nums">
                    {ratingData.estimatedGames.toLocaleString()}
                  </span>
                  <span className="text-muted-foreground text-xs">{t("player.games")}</span>
                </span>
              )}
              {ratingData && ratingData.longestWinStreak > 1 && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-card/50 border border-border/40 font-mono">
                  <Trophy className="w-3.5 h-3.5 text-amber-500" />
                  <span className="text-foreground font-bold tabular-nums">
                    {ratingData.longestWinStreak}
                  </span>
                  <span className="text-muted-foreground text-xs">{t("player.winStreak")}</span>
                </span>
              )}
            </>
          ) : (
